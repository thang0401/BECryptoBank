...
public interface SavingAccountDAO extends JpaRepository<SavingAccount,String> {
    ...
    @Query("SELECT new com.cryptobank.backend.DTO.SavingAccountDTO(s.id ,s.user.id,s.user.firstName,s.user.lastName,s.heirStatus,s.balance,s.term.amount_month,s.term.type,s.createdDate,s.createdDate) FROM SavingAccount s")
    public List<SavingAccountDTO> findAllDTO();
